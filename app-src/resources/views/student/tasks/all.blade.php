    ...
    <div class="row">
        <div class="col-md-12" id="active">
            <div class="default-according" id="accordion">
                @foreach ($gruposNaoEntregues as $group)
                <div class="card">
                    <div class="card-header card-header-tarefa" id="heading_{{$group->task->task_id}}"
                        ...
                        <div class="row">
                            <div class="col-7 col-md-7 col-xl-8">
                                ...
                                <h5 class="validade-mobile">Validade {{$group->task->date->task_date_due}}</h5>
                            </div>
                            <div class="float-md-end col-5 col-md-5 col-xl-4">
                                <div class="row justify-content-end">
                                    <h6 class="validade mt-2 col-md-7">Validade {{$group->task->date->task_date_due}}</h6>
                                    <button
                                        onclick="window.location.href = '{{ route('studentsTaskDetailed', $group->group_id) }}'"
                                        class="btn btn-action p-2 p-md-2 col-12 col-md-5">Ver Tarefa</button>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="col-sm-12" id="done" style="display:none">
            <div class="default-according" id="accordion1">
                @foreach ($gruposEntregues as $group)
                <div class="card">
                    <div class="card-header card-header-tarefa" id="heading_{{$group->task->task_id}}"
                        data-bs-toggle="collapse" data-bs-target="#collapse_{{$group->task->task_id}}" aria-expanded="true"
                        aria-controls="collapse_{{$group->task->task_id}}">
