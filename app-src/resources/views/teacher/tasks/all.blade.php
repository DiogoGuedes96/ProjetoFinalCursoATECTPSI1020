    ...
    <div class="row">
                        ...
                        <div class="row">
                            ...
                            <div class="col-4 col-xl-6">
                                <div class="row justify-content-end">
                                    ...
                                    <button
                                        onclick="window.location.replace('{{route('teachersTask',$task->task_id)}}')"
                                        class="btn btn-action-task p-2 p-md-2 col-12 col-md-6 col-lg-6 col-xl-3">Ver
                                        Tarefa</button>
                                    <button
                                        onclick="window.location.replace('{{route('teachersTaskEdit',$task->task_id)}}')"
                                        class="btn btn-edit p-0 mx-xl-3 my-2 my-sm-0 col-12 col-md-3 col-xl-1">
                                        <i data-feather="edit"></i></button>
                                    <button
                                        onclick="window.location.replace('{{route('teachersTaskDelete',$task->task_id)}}')"
        ...
        <div class="col-sm-12" id="done" style="display:none">
            <div class="default-according" id="accordion1">
                @foreach($teacher->teacher_tasks as $task)
                @if ($task->task_teacher_status == "YES" && ($task->task_status == "Evaluated" || $task->task_status == "Finished"))
                <div class="card">
                    <div class="card-header card-header-tarefa collapsed" id="heading{{$task->task_id}}"
                        data-bs-toggle="collapse" data-bs-target="#collapse{{$task->task_id}}" aria-expanded="true"
                        aria-controls="collapse{{$task->task_id}}">
        ...
        <div class="col-sm-12" id="teacher_status" style="display:none">
            <div class="default-according" id="accordion1">
                @foreach($teacher->teacher_tasks as $task)
                @if ($task->task_teacher_status == "NO" && ($task->task_status == "Evaluated" || $task->task_status == "Finished"))
                <div class="card"> <!-- task-red -->
                    <div class="card-header card-header-tarefa collapsed" id="heading{{$task->task_id}}"
                        data-bs-toggle="collapse" data-bs-target="#collapse{{$task->task_id}}" aria-expanded="true"
                        aria-controls="collapse{{$task->task_id}}">
                        <div class="row">
                            ...
                            <div class="col-4 col-xl-6">
                                <div class="row justify-content-end">
                                    <button
                                        onclick="window.location.replace('{{route('teacherTaskEvaluation',$task->task_id)}}')"
