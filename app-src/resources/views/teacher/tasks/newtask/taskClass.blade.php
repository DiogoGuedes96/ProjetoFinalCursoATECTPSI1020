    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer text-center">
                <a
                    onclick="teacherRedirect(document.getElementById('select-turma').options[document.getElementById('select-turma').selectedIndex].value)"><button
...
<script>
    ...
    $('#select_turma_modal').on('hide.bs.modal', function (e) {
        window.location.replace('{{ route('teacherDashboard') }}');
    });
    function teacherRedirect(turma) {
        if (turma == "0") {
            document.getElementById("alert-no-class-selected").style.display = "block";
        }else{
            var url = '{{ route("teacherNewTaskGroup", [":turma", ":task", ":ufcd"]) }}';
