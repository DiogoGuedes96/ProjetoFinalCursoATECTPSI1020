...
<div class="container-fluid">
    <div class="page-header py-3">
        <div class="col-md-12 project-list">
            <div class="row">
                ...
                <div class="col-6 p-0">
                    <a href="{{route('coordinatorTasks')}}"><button class="btn btn-action">
                        Voltar <i data-feather="chevron-left"></i>
                    </button></a>
                    <a href="{{ route('coordinatorTasks') }}"><button
                            class="btn btn-action btn-action-mobile mx-1">
                            <i data-feather="chevron-left"></i>
                        </button></a>
                    <a href="{{route('coordinatorTaskDelete',$task->task_id)}}"><button class="btn btn-trash mx-2"><i data-feather="trash-2"></i></button>
                    <a href="{{route('coordinatorTaskEdit',$task->task_id)}}"><button class="btn btn-edit mx-1"><i data-feather="edit"></i></button></a>
                </div>
            </div>
        </div>
        <hr>
    ...
    @if ($task->task_status == "unassigned")
        <div class="py-2">
            <button onclick="window.location.replace('{{route('coordinatorNewTaskClass', $task->task_id)}}')" class="btn btn-action">
                Criar Grupos
            </button>
        </div>
    @else
...
<script type="text/javascript">
   ...
   $("#button_1").on("click", function(e) {
        e.preventDefault();
        $.ajax({type: "get",
            url: "{{route('coordinatorTaskDownload',$task->task_id)}}",
